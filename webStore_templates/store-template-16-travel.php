<?php
/**
 * TAPIFY - WhatsApp Store Template 16 — "Desi Miles Tour & Travels" (v2, light theme)
 * Converted from webStoreTemps/Theme 8. Renders shared $store/$categories/$products.
 */
$cfg   = $templateConfig ?? [];
$asset = '/webStore_templates/assets/store_template_16';
$defC  = $cfg['default_colors'] ?? ['primary' => '#1e88a8', 'secondary' => '#0d3b4a', 'accent' => '#f4a340'];
...
$cta         = $cfg['product_cta'] ?? 'Book Now';
$enableTranslate = !array_key_exists('enable_translate', $store) || !empty($store['enable_translate']);
$seoTitle  = !empty($store['seo_title']) ? htmlspecialchars($store['seo_title']) : "Tour Packages | $shopName";
...
body{background:#f4f8fa;color:#1a2a35;font-family:'Poppins',sans-serif;overflow-x:hidden;}
.d-none{display:none!important;}
:root{--tp:<?= $P ?>;--ts:<?= $S ?>;--ta:<?= $A ?>;}

.tp-bg-vector{position:fixed;pointer-events:none;z-index:0;opacity:.05;}
.tp-bg-v1{top:5%;right:-3%;width:260px;height:260px;background:var(--ta);border-radius:50%;filter:blur(80px);}
.tp-bg-v2{bottom:15%;left:-5%;width:200px;height:200px;background:var(--tp);border-radius:50%;filter:blur(60px);}

.tp-nav{display:flex;align-items:center;justify-content:space-between;padding:12px 24px;background:rgba(255,255,255,.92);backdrop-filter:blur(10px);position:sticky;top:0;z-index:100;box-shadow:0 1px 4px rgba(0,0,0,.05);}
.tp-nav-brand{display:flex;align-items:center;gap:10px;text-decoration:none;color:#1a2a35;}
...
.tp-nav-brand span{font-size:17px;font-weight:600;color:#1a2a35;}
.tp-nav-right{display:flex;align-items:center;gap:16px;}
.tp-lang-select{padding:6px 10px;border:1px solid #d0dce3;border-radius:6px;font-size:13px;color:#555;background:#fff;cursor:pointer;}
.tp-cart-btn{position:relative;background:none;border:none;cursor:pointer;color:#1a2a35;font-size:22px;}
...
.tp-section-title{font-size:22px;font-weight:700;color:#1a2a35;margin:28px 0 16px;padding:0 24px;position:relative;z-index:1;}
...
.tp-cat-circle{width:72px;height:72px;border-radius:50%;overflow:hidden;border:3px solid transparent;margin:0 auto 8px;background:#e8f0f4;}
...
.tp-carousel-arrow{position:absolute;top:50%;transform:translateY(-50%);width:32px;height:32px;border-radius:50%;background:#fff;border:1px solid #d0dce3;cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:14px;color:#333;z-index:5;box-shadow:0 2px 6px rgba(0,0,0,.06);}
...
.tp-product-card{background:#fff;border-radius:12px;overflow:hidden;display:flex;flex-direction:column;border:1px solid #e3edf2;transition:transform .3s,box-shadow .3s;}
.tp-product-card:hover{transform:translateY(-3px);box-shadow:0 6px 20px rgba(30,136,168,.12);}
.tp-product-img{width:100%;aspect-ratio:1;overflow:hidden;background:#f4f8fa;}
.tp-product-img img{width:100%;height:100%;object-fit:cover;}
.tp-product-img .no-img{width:100%;height:100%;display:flex;align-items:center;justify-content:center;color:#c0d0d8;font-size:2rem;}
.tp-product-info{padding:12px 14px;flex-grow:1;display:flex;flex-direction:column;}
.tp-product-name{font-size:14px;font-weight:600;color:#1a2a35;margin-bottom:2px;}
.tp-product-cat{font-size:12px;color:#8899a3;margin-bottom:8px;}
.tp-product-price{font-size:15px;font-weight:700;color:#1a2a35;margin-bottom:10px;}
.tp-product-price del{color:#aab8c0;font-size:12px;font-weight:400;margin-left:4px;}
...
.tp-view-more{display:inline-flex;align-items:center;gap:0;background:#fff;border:1px solid #d0dce3;border-radius:50px;overflow:hidden;text-decoration:none;transition:transform .2s,box-shadow .2s;}
.tp-view-more:hover{transform:scale(1.03);box-shadow:0 4px 12px rgba(30,136,168,.15);}
...
.tp-filter-sidebar input[type="search"],.tp-filter-sidebar input[type="number"],.tp-filter-sidebar select{width:100%;padding:10px 12px;border:1px solid #d0dce3;border-radius:8px;background:#fff;color:#333;font-size:14px;outline:none;}
...
.tp-filter-section h4{font-size:15px;font-weight:700;color:#1a2a35;margin-bottom:12px;padding-bottom:8px;border-bottom:1px solid #e3edf2;}
...
.tp-cart-head{display:flex;align-items:center;justify-content:space-between;padding:18px 20px;border-bottom:1px solid #e3edf2;color:#1a2a35;}
...
.tp-ci{display:flex;gap:12px;padding:12px 0;border-bottom:1px solid #f0f4f6;color:#1a2a35;}
.tp-ci img{width:56px;height:56px;object-fit:cover;border-radius:8px;background:#f0f4f6;}
...
.tp-cart-foot{padding:16px 20px;border-top:1px solid #e3edf2;color:#1a2a35;}
...
<div class="main-content mx-auto w-100">
  <nav class="tp-nav">
    ...
    <div class="tp-nav-right">
      ...
      <button class="tp-cart-btn" onclick="tpOpenCart()"><i class="fas fa-shopping-bag"></i><span class="tp-cart-badge" id="tpCartCount">0</span></button>
    </div>
  </nav>
  ...
  <div class="tp-filtered-view" id="tpFilteredView">
    <div class="tp-filter-layout">
      <div class="tp-filter-sidebar">
        <div class="tp-search-wrap"><i class="fas fa-search"></i><input type="search" id="tpSearch2" placeholder="Search for Packages" oninput="tpSearch2()"></div>
        ...
        <div class="tp-filter-section"><h4>Date Posted</h4><?php foreach (['3_days'=>'3 Days Ago','1_week'=>'1 Week Ago','1_month'=>'1 Month Ago','6_months'=>'6 Months Ago','1_year'=>'1 Year Ago'] as $val=>$lbl): ?><label><input type="radio" name="tpDateFilter2" value="<?= $val ?>" onchange="tpApplyDate2('<?= $val ?>')"> <?= $lbl ?></label><?php endforeach; ?></div>
  ...
  <footer style="background:#0d3b4a;text-align:center;padding:24px 20px;color:#ccc;font-size:14px;position:relative;z-index:1;">
    <?php if ($address): ?><div class="mb-2" style="color:#a0b8c2;"><i class="fas fa-map-marker-alt"></i> <?= $address ?></div><?php endif; ?>
    <div>© Copyright <?= date('Y') ?> Tapify. All Rights Reserved.</div>
  </footer>
</div>

<div id="tpCartOverlay" onclick="tpCloseCart()"></div>
<div id="tpCartDrawer"><div class="tp-cart-head"><h5>Your Cart</h5><button class="tp-cart-close" onclick="tpCloseCart()">×</button></div><div class="tp-cart-items" id="tpCartItems"></div><div class="tp-cart-foot" id="tpCartFoot"></div></div>
