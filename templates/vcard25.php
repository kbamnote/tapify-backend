<?php
/**
 * Tapify vCard Template: vcard25
 * Pharma / Medical theme — Mint Green
 */
...
$profileImg = !empty($vcard['profile_image']) ? imgUrl($vcard['profile_image']) : 'https://ui-avatars.com/api/?name='.urlencode($fullName).'&size=200&background=16a34a&color=ffffff';
$coverImg = !empty($vcard['cover_image']) ? imgUrl($vcard['cover_image']) : 'https://images.unsplash.com/photo-1587854692152-cbe660dbde88?w=500&q=80';
...
.banner-overlay{position:absolute;inset:0;background:linear-gradient(to bottom,rgba(5,46,22,0.35) 0%,rgba(5,46,22,0.8) 100%)}
...
.accent-bar{position:absolute;bottom:0;left:0;right:0;height:3px;background:linear-gradient(90deg,var(--accentD),var(--accent),#4ade80,var(--accent),var(--accentD));background-size:200% 100%;animation:bar-slide 3s linear infinite}
...
.avatar{width:92px;height:92px;border-radius:50%;object-fit:cover;border:4px solid #fff;display:block;box-shadow:0 0 0 3px var(--accent),0 6px 20px rgba(22,163,74,0.2)}
.cta-btn{display:inline-flex;align-items:center;gap:7px;background:var(--accent);color:#fff;font-size:0.72rem;font-weight:700;letter-spacing:0.06em;padding:8px 20px;border-radius:24px;text-decoration:none;margin-bottom:14px;box-shadow:0 4px 14px rgba(22,163,74,0.3);transition:background 0.2s,transform 0.2s}
...
.soc{width:34px;height:34px;border-radius:50%;background:rgba(22,163,74,0.1);border:2px solid rgba(22,163,74,0.35);color:var(--accent);display:flex;align-items:center;justify-content:center;font-size:0.8rem;text-decoration:none;transition:background 0.2s,transform 0.2s}
.soc:hover{background:var(--accent);color:#fff;transform:translateY(-2px)}

/* BIO */
.bio-section{padding:18px 22px;background:var(--accentXL)}
.bio-box{border-left:4px solid var(--accent);padding:14px 16px;background:linear-gradient(135deg,#dcfce7,var(--accentXL));border-radius:0 12px 12px 0;box-shadow:0 2px 8px rgba(22,163,74,0.1)}
...
.c-item{background:#fff;border-radius:12px;padding:12px 10px;display:flex;align-items:center;gap:10px;text-decoration:none;border:1.5px solid var(--accentL);box-shadow:0 2px 6px rgba(22,163,74,0.08);transition:box-shadow 0.2s,transform 0.2s,border-color 0.2s;overflow:hidden}
.c-item:hover{box-shadow:0 4px 14px rgba(22,163,74,0.22);transform:translateY(-2px);border-color:var(--accent)}
...
.srv-card{flex:0 0 calc(50% - 6px);background:#fff;border-radius:12px;overflow:hidden;border:1.5px solid var(--accentL);box-shadow:0 2px 8px rgba(22,163,74,0.1)}
.srv-card img{width:100%;height:105px;object-fit:cover;display:block}
.srv-no-img{width:100%;height:105px;background:linear-gradient(135deg,#dcfce7,var(--accentL));display:flex;align-items:center;justify-content:center;color:var(--accent);font-size:1.6rem}
...
.carousel-btn{position:absolute;top:50%;transform:translateY(-50%);width:30px;height:30px;border-radius:50%;background:var(--accent);color:#fff;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:0.72rem;z-index:2;box-shadow:0 2px 8px rgba(22,163,74,0.3)}
...
<div class="card-wrap">
  ...
  <div class="banner">
    ...
    <div class="banner-content">
      <div class="banner-tag"><i class="fas fa-pills"></i> <?= htmlspecialchars($vcard['occupation'] ?? 'Pharma & Medical') ?></div>
      <div class="banner-big"><?= htmlspecialchars($vcard['company'] ?? 'Your Health, Our Priority') ?></div>
    </div>
  ...
  <div class="profile-section fade-in">
    ...
    <a href="tel:<?= htmlspecialchars($vcard['phone']) ?>" class="cta-btn"><i class="fas fa-phone-alt"></i> Order Now</a>
    <?php endif; ?>
  ...
  <div class="section fade-in">
    <div class="sec-h">
      <div class="sec-ico"><i class="fas fa-capsules"></i></div>
      <span class="sec-title">Products &amp; Services</span>
    </div>
    <div class="carousel-wrap">
      ...
      <div class="carousel-track" id="track<?= $vcardId ?>">
        ...
        <div class="srv-card">
          ...
          <?php else: ?>
          <div class="srv-no-img"><i class="fas fa-heartbeat"></i></div>
          <?php endif; ?>
          <div class="srv-body">
