...
function embeddableMapUrl($url) {
    ...
    $query = '';
    if (preg_match('/@(-?\d+\.\d+),(-?\d+\.\d+)/', $url, $m)) {            // @lat,lng
        $query = $m[1] . ',' . $m[2];
    } elseif (preg_match('/!3d(-?\d+\.\d+)!4d(-?\d+\.\d+)/', $url, $m)) {  // !3dlat!4dlng
        $query = $m[1] . ',' . $m[2];
    } elseif (preg_match('#/maps/place/([^/@?]+)#', $url, $m)) {          // /maps/place/<name>
        $query = urldecode($m[1]);
    } elseif (preg_match('/[?&](?:q|query)=([^&]+)/', $url, $m)) {        // ?q= / ?query=
        $query = urldecode($m[1]);
    } else {
        // Short link we can't parse (goo.gl) — leave untouched.
        return $url;
    }
...
if (!empty($_GET['preview'])) {
    ...
    if (preg_match('/^vcard(0[1-9]|[12][0-9]|28)$/', $previewId) ||
        preg_match('/^vcard([1-9]|[1-5][0-9]|6[0-9]|7[0-9])$/', $previewId)) {
...
if (!preg_match('/^vcard(0[1-9]|[12][0-9]|28)$/', $templateId) &&
    !preg_match('/^vcard([1-9]|[1-5][0-9]|6[0-9]|7[0-9])$/', $templateId)) {
    $templateId = 'vcard01';
}
